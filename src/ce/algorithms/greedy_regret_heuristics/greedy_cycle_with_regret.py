...
def get_2_regret(edges: List[Tuple[int, int]], tsp: TSP, node_index: int) -> Tuple[int, Tuple[int, int]]:
    ...
    return second_best_edge[0] - best_edge[0], int(best_edge[1])

def extend_cycle(cycle: List[int], tsp: TSP):
    if len(cycle) == 1:
        current_node = cycle[0]
        next_node = np.argmin(tsp.distances[current_node, :])
    ...
    regrets = [(node, get_2_regret(edges, tsp, node)) for node in nodes_indices]
...
def greedy_cycle_with_regret(tsp: TSP, start_node: int, with_debug=None) -> List[int]:
